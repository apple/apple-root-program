...
**Detailed Controls Report (DCR)**: A report, or its equivalent, that documents a CA's controls and the auditor's procedures and results for testing those controls.
...
| :------ | :--- | :----------------- |
| 2.0 | 2026-08-01 | Added Appendix A defining Trust Purposes and their required Extended Key Usage values. Separated the SSL Baseline and Network Security audit criteria, and added Network Security audits for S/MIME and VMC. Effective 2026-08-01, Subordinate CA Certificates MUST contain an Extended Key Usage extension and MUST NOT assert anyExtendedKeyUsage, Externally Operated Subordinate CAs require Apple approval before issuance, and Root Inclusion Requests require a minimum of RSA 4096-bit or ECDSA 384-bit. Effective 2027-02-01, S/MIME subscriber certificates MUST include at least one rfc822Name. Effective 2027-07-01, Subordinate CA Certificates MUST be dedicated to a single Trust Purpose and each CP/CPS MUST be a Markdown document scoped to a single Trust Purpose. Detailed Controls Report attestation requirements apply to audit periods starting on or after 2027-07-01. |
...
CAs capable of issuing TLS certificates (i.e. contain id-kp-serverAuth EKU with non-EV Policy OID) MUST be audited at least annually against the Current Version (and MAY use a newer version if effective during the audit period) of at least one of the following sets of criteria. Every criterion in the chosen set MUST be met:

**WebTrust Set 1 (Preferred)**
* "WebTrust Principles and Criteria for Certification Authorities"
...
**WebTrust Set 2 (Accepted)**
* "WebTrust Principles and Criteria for Certification Authorities"
* "WebTrust Principles and Criteria for Certification Authorities - SSL Baseline with Network Security"

**ETSI Set 1 (Accepted on a case-by-case basis):** "ETSI EN 319 411-1" LCP and DVCP

**ETSI Set 2 (Accepted on a case-by-case basis):** "ETSI EN 319 411-1" LCP and OVCP

**ETSI Set 3 (Accepted on a case-by-case basis):** "ETSI EN 319 411-1" NCP and EVCP

#### 1.1.2 EV TLS CA Owners

CAs capable of issuing EV TLS (i.e. contain id-kp-serverAuth EKU with EV Policy OID) certificates MUST be audited at least annually against the Current Version (and MAY use a newer version if effective during the audit period) of at least one of the following sets of criteria. Every criterion in the chosen set MUST be met:
...
**ETSI Set 1 (Accepted on a case-by-case basis):** "ETSI EN 319 411-1" NCP and EVCP

#### 1.1.3 S/MIME CA Owners

CAs capable of issuing S/MIME certificates (i.e. contain id-kp-emailProtection EKU) MUST be audited at least annually against the Current Version (and MAY use a newer version if effective during the audit period) of at least one of the following sets of criteria. Every criterion in the chosen set MUST be met:
...
**ETSI Set 1 (Accepted on a case-by-case basis):** "ETSI TS 119 411-6" LCP

**ETSI Set 2 (Accepted on a case-by-case basis):** "ETSI TS 119 411-6" NCP

**ETSI Set 3 (Accepted on a case-by-case basis):** "ETSI TS 119 411-6" NCP+

#### 1.1.4 VMC CA Owners

CAs capable of issuing VMC certificates (i.e. contain BrandIndicatorforMessageIdentification EKU) MUST be audited at least annually against the Current Version (and MAY use a newer version if effective during the audit period) of all of the following criteria:
...
* separate reports supplied to the Apple Root Program within 90 calendar days of such Events concluding.
...
Apple reserves the right to require a CA Owner to complete additional audit engagements, appoint or reject an auditor, or require submission of a Detailed Controls Report.
...
The Detailed Controls Report MUST, at a minimum, contain the following core elements for each tested requirement:
...
For WebTrust audits, the CA Owner MUST ensure the auditor prepares the Detailed Controls Report in accordance with the Detailed Controls Report process defined by CPA Canada.
For ETSI audits, the CA Owner MUST ensure the auditor prepares an equivalent report that contains the four core elements above.
The Detailed Controls Report is scoped to the same systems, controls, and certificates covered by the corresponding audit.
The Detailed Controls Report MAY be a single, consolidated document covering all audit criteria applicable to the CA Owner for the audit period.
...
Apple MAY request any completed Detailed Controls Report required under this section, at any time. CA Owners MUST retain all such reports for seven years (or less where applicable law requires earlier destruction) and MUST provide any that Apple requests within 30 calendar days.
The CCADB Case Comment attestation that the Detailed Controls Report (or its equivalent) has been generated MUST be added at the time the CA Owner submits the corresponding audit report to CCADB.
Apple will treat any Detailed Controls Report provided to Apple as confidential. Apple will not publish it and will not disclose it to third parties except as required by law. Redactions are acceptable only where required by applicable law. Where redactions are made, the CA Owner MUST state the legal basis.
...
Citing the governing Baseline Requirements, RFC, or other standard as the source of a requirement is permitted and does not violate this requirement; the CA Owner MUST additionally describe its own corresponding practice rather than reproduce the cited text in full.

3. **Trust Purpose Scoping:**

   * Each CP/CPS document MUST be scoped to a single Trust Purpose (as defined in Appendix A), except that:
       * A CP/CPS document MAY combine "Server Authentication" and "Legacy TLS".
       * A CP/CPS document MAY combine "Secure Email" and "Legacy S/MIME".
   * The Trust Purpose(s) and required EKU(s) MUST be stated in Section 1.4 of the CP/CPS.
   * A Root CA Certificate hierarchy supporting multiple Trust Purposes MUST provide a distinct CP/CPS document for each Trust Purpose, subject to the combined-pair exceptions above.
   * Multiple Root CA Certificate hierarchies MAY reference the same CP/CPS document if they share the same Trust Purpose or combined pair.
   * Certificates and hierarchies not aligned to an Apple-recognized Trust Purpose MUST be covered by at least one CP/CPS document (see Appendix A for Subordinate CA Certificates signed before 2026-08-01 and Section 1.7 for those signed on or after 2026-08-01).

### 1.4 Communication Requirements

* CA Owners MUST maintain up to date contact details in the CCADB.
* CA Owners are accountable for staying current on discussions in the following forums (where applicable) and implementing any changes necessary to conform with communications from them:
...
* Root Inclusion Requests MUST only contain Root CA Certificates that have a minimum key size of RSA 4096-bit or ECDSA 384-bit.
...
A Subordinate CA Certificate signed on or after 2026-08-01 MUST contain an Extended Key Usage extension and MUST NOT assert anyExtendedKeyUsage (2.5.29.37.0).

A Subordinate CA Certificate signed on or after 2026-08-01 and before 2027-07-01 MUST meet either (a) or (b):

* (a) It is dedicated to a single Trust Purpose as defined in [Appendix A](#appendix-a-trust-purposes).
* (b) All of the following are true:
  * it is issued under a Root CA Certificate that supports multiple Trust Purposes;
  * it contains no Extended Key Usage associated with a Trust Purpose defined in Appendix A; and
  * its use case (for example, Document Signing) is covered by a CP/CPS within the scope of the CA Owner's annual audit.

A Subordinate CA Certificate signed on or after 2027-07-01 MUST be dedicated to a single Trust Purpose as defined in [Appendix A](#appendix-a-trust-purposes).

For the purposes of this Section, a renewal, re-key, or cross-sign produces a new Subordinate CA Certificate whose signing date is the date of that issuance. Apple MAY require a CA Owner to discontinue issuance from a Subordinate CA Certificate qualifying under (b) at any time.
...
* CA Owners SHOULD be aware that issuance of a precertificate or certificate chaining up to an included CA Certificate constitutes authorization by the CA Owner for that certificate.
* CA Owners SHOULD be aware that participation in the Apple Root Program constitutes a Root Certificate distribution agreement, as referenced in Section 9.9 of the TLS Baseline Requirements or S/MIME Baseline Requirements, between the CA Owner and Apple.
...
A Subordinate CA Certificate in these hierarchies MUST NOT contain the same public key as any other certificate that asserts an EKU the Subordinate CA Certificate does not contain.
...
*Note*: TLS subscriber certificates MUST comply with the [Apple Certificate Transparency Policy](https://support.apple.com/103214) to be accepted for connection establishment on Apple devices.

### 2.3 S/MIME Trust Purposes

For Secure Email and Legacy S/MIME Trust Purposes:

Effective 2027-02-01, all newly signed Subscriber certificates that contain the id-kp-emailProtection EKU MUST include at least one rfc822Name in the subjectAltName extension.

### 2.4 VMC Trust Purpose

For the VMC Trust Purpose:

Apple's platforms recognize Verified Mark Certificates that meet the requirements in Appendix A.

## 3. Incidents

Failure to comply with this policy in any way is considered an incident. Section 1.3 requires compliance with the applicable Baseline Requirements, the CCADB Policy, and the CA Owner's own Policy Documents. Failing any of these is therefore an incident.
...
* the domain names and/or top-level domains for which leaf certificates will validate are constrained to a subset of what is otherwise allowed for the CA Certificate's designated Trust Purpose
...
* **VMC Requirements**: AuthIndicators Working Group Minimum Security Requirements for Issuance of Mark Certificates

**Additional Notes:**

* For all Trust Purposes, Subordinate CA Certificates (including Cross-Signed CA Certificates) signed on or after 2026-08-01 MUST contain only the EKU(s) specified for that Trust Purpose.
* For all Trust Purposes, subscriber (end-entity) certificates MUST contain one or more of the EKU(s) specified for that Trust Purpose.
...
* Subscriber certificates issued under a Trust Purpose defined in this Appendix and signed before 2027-07-01 MAY contain EKU combinations that do not conform to the single Trust Purpose scoping defined in this Appendix, provided the certificate otherwise complies with all applicable Baseline Requirements.
* Subordinate CA Certificates signed before 2026-08-01 MAY be multi-purpose. Subscriber certificates issued from them MAY contain EKUs not defined in this Appendix, provided such use cases are covered by a compliant audit against the WebTrust or ETSI criteria listed in Section 1.1, or another framework Apple has approved in writing, and those details are documented within a corresponding CP/CPS.
